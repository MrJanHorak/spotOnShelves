...
function App() {
  ...
  const [result, setResult] = useState<CalculationResult>({ shelves: [], measurements: [], instructions: [] });
  ...
  useEffect(() => {
    ...
    if (validationErrors.length === 0 && shelves.length > 0) {
      const calculationResult = calculateOptimalPlacement(wall, shelves, obstructions, settings.alignment);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center">
            <p className="text-gray-400 text-sm">
              Spot On Shelves - Your trusted companion for precise shelf installation
            </p>
            <p className="text-gray-500 text-xs mt-2">
              Always prioritize safety and consult professionals for complex installations
            </p>
            <p className="text-gray-500 text-xs mt-2">
