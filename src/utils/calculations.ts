...
export function calculateOptimalPlacement(
  ...
  sortedShelves.forEach((shelf) => {
    ...
    while (attempts < maxAttempts) {
      ...
      const hasConflict = usedSpaces.some(
        (space) =>
          !(
            shelfSpace.x2 <= space.x1 ||
            shelfSpace.x1 >= space.x2 ||
            shelfSpace.y2 <= space.y1 ||
            shelfSpace.y1 >= space.y2
          )
      );
  ...
  const measurements = placements.map(
    (placement, index) =>
      `Shelf ${index + 1}: ${placement.distanceFromLeft.toFixed(
        1
      )} from left wall, ${placement.distanceFromFloor.toFixed(1)} from floor`
  );
...
export function calculateMaterials(
  ...
  shelves.forEach((s, i) => {
    ...
    perShelf.push({ id, width: s.width, brackets: b, screws: sc, anchors: an });
    totalBrackets += b;
    totalScrews += sc;
    totalAnchors += an;
  });
